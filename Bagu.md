...
这份面试八股文是我自己简单总结的。我是个保研边缘人，在2024年暑假陷入了几乎不可能保研的处境，于是开始了秋招之旅。万幸的是，由于自己的学生工作还算过得去，2024年9月幸运拿到了学校研支团推免资格。保研后，我认真反思了秋招阶段遇到的的问题，发现八股文这一块学校几乎不讲，而且自己的整个技术栈也很薄弱，因此继续深入学习，总结出这份八股文，以便未来所需。

学习八股文，一般采用三个步骤，第一步提炼出知识点，熟悉那个知识点的名词，第二步寻找对应名词的相关面试题，做到心中有数，第三步带着这些问题进行体系化的学习，记笔记并梳理思维导图。

当然，面试前夕最有效的方法则是阅读面经，直接背诵。
...
1. 浏览器的同源策略：协议、域名、端口号都相同的两个页面才是同源的，否则就是跨域。**三者有一个不同就是跨域！**
...
可以这样记：**拥塞控制范围大，流量控制范围小。** 拥塞控制是让网络能够承受现有的网络负荷，是一个全局性的过程，涉及所有的主机、所有的路由器。相反，流量控制是指点对点的控制，只涉及到发送方和接收方。（**接收端控制发送端**）
...
此外，**拥塞控制**是为了防止过多的数据注入到网络中，这样可以使网络中的路由器或链路不会过载。**流量控制**是为了防止发送方发送数据过快，这样接收方来不及处理。

![14](./imgs/14.png)

实现拥塞控制，具有下面的四种办法：**慢开始、拥塞避免（这两个一组）、快重传、快恢复（这两个一组）。**

## 6 HTTP和HTTPS区别

1. 加密。HTTP 是超文本传输协议，信息是明文传输，存在安全风险的问题。HTTPS 则解决 HTTP 不安全的缺陷，在 TCP 和 HTTP 网络层之间加入了 SSL/TLS 安全协议，使得报文能够加密传输。

2. 握手。HTTP 连接建立相对简单， TCP 三次握手之后便可进行 HTTP 的报文传输。而 HTTPS 在 TCP 三次握手之后，还需进行 SSL/TLS 的握手过程，才可进入加密报文传输。

3. 端口。两者的默认端口不一样，HTTP 默认端口号是 80，HTTPS 默认端口号是 443。
...
如图所示的7个步骤要搞清楚，如果搞不清楚看https://www.bilibili.com/video/BV1KY411x7Jp/?spm_id_from=333.337.search-card.all.click&vd_source=3f8c5c4e56e5c611151b798b557016c9

HTTPS在得到会话密钥（便便色）前，使用的非对称加密进行握手和传输。算出会话密钥后，使用会话密钥+对称加密进行传输。不使用非对称加密的原因是因为非对称加密速度太慢。

## 8 HTTP/1.1 HTTP/2 和 HTTP/3区别

![alt text](image-36.png)

**HTTP1.1，1997年**

这是互联网的第一个真正意义的HTTP标准版本，它的核心在于**一次一份。** 在发送HTTP请求后，需要收到服务器返回的HTTP相应，才能发送下一个HTTP请求。那么问题来了，我们访问一个网站，需要加载很多资源，而往往一个资源没有收到，后面的资源就会无法加载。这就造成了著名的**HTTP队头阻塞**问题。

为了减少HTTP的队头阻塞问题，HTTP1.1有一个叫做“管线化”的技术，它可以允许单个连接发送多个请求。但它很傻逼，因为响应的时候必须按照发送的顺序接收，这就造成了很大的执行难度。实际中使用的较少。

此外，对于开发们来说，把多个图片拼成一个图片的这种“精灵图”，或者把多个js文件合并成一个js文件，甚至说把js和css全都合并到html里去，或者说把图片按照文字编码的“dataURL”的这些“黑科技”，正是为了减少HTTP请求的次数。

此外，还可以用“域名分片”这种办法，例如：image1.baidu.com, image2.baidu.com...就是把资源放到不同的域名下，这样可以增加浏览器的并行请求。**因为限制请求数实际上是针对某个域名而言的。**
...
HTTP2的核心在于**多路复用。** HTTP2可以在一个连接上同时传输多个资源，而且不用按照顺序接收。这是因为，请求和响应报文被划分成各个不同的帧，这个帧可以分为**首部帧和数据帧**，其实就是把原来的HTTP报文拆分为**首部+实体**两部分，这样原本的HTTP报文就不再是原来的报文了，反而更像数据链路层中的“帧”。这个帧中，最重要的是有个“**流标识符**”，有了这个流标识符，就可以区分数据，使得数据不一定按照顺序抵达对方那里。这样就解决了HTTP1.1的HTTP队头阻塞问题。

HTTP2还有一个特点就是**头部压缩**，HTTP1.1的头部是文本格式，而HTTP2的头部是二进制格式，这样就可以减少头部的大小。此外，它通过一定的压缩算法，进一步减少头部的大小。

此外，还有一个傻逼功能叫做“**服务器推送**”，就是服务器可以在客户端请求之前，主动推送资源给客户端。这样可以减少客户端的请求次数。但也有弊端，就是服务器推送的资源可能是客户端不需要的，这样就浪费了资源，还可能导致非对称DDos攻击。

**HTTP3，2019年**

HTTP2解决了HTTP队头阻塞的问题，但是它并没有解决**TCP队头阻塞**的问题。

HTTP3的一大核心是“整合”。通过QUIC协议，把TCP和TLS的握手整合到了一起。**HTTP3必须加密。**

![16](./imgs/16.png)

QUIC把应用层的数据封装成为一个QUIC帧，这个帧再被封装成为一个QUIC数据包。这个数据包通过UDP传输，这样就不会有TCP队头阻塞的问题了。

此外，在QUIC数据包中，引入了一个“**连接ID**”，如果网络发生全面改变，比如WiFi切换到4G，如果这个连接ID是不变的，就直接建立连接，而不用再次握手，这就是QUIC快的原因之一。

## 9 HTTP常见状态码

看视频。。。https://www.bilibili.com/video/BV1jY411u7Zn/?spm_id_from=333.788&vd_source=3f8c5c4e56e5c611151b798b557016c9

## 10 常见的HTTP方法，例如GET和POST有什么区别

常见的HTTP方法：**GET、POST、PUT**、DELETE、HEAD、**OPTIONS**、TRACE、CONNECT。在HTTP1.0的时候只有GET、POST和HEAD。
...
1. GET通过HTTP头或者URL传递参数，POST除了这两个外，还能通过请求体传递参数。**需要注意的是，如果GET携带了请求体，那么浏览器会自动忽略掉这部分请求体，不过当你用Nodejs发送数据时，会带上这个请求体。**

2. 由于GET是通过URL传递参数，所以GET的参数有长度限制（2K），而POST没有。

3. GET请求会被浏览器缓存，POST不会。

4. GET请求会被浏览器保存在浏览器历史记录中，POST不会。

5. GET请求只能进行url编码，只接受ASCII字符，而POST没有限制。

另外，HEAD是向服务器索与GET请求相一致的响应，只不过响应体将不会被返回。这一方法可以再不必传输整个响应内容的情况下，就可以获取包含在响应小消息头中的元信息。

# 操作系统篇

## 1 进程与线程的区别

传统的进程中，进程是程序执行流的最小单位；引入了线程的概念之后，一个程序里可以包含多个线程（如QQ聊天、传文件同时并发运行），CPU分别轮流为不同程序的不同线程服务，线程成为了程序执行流的最小单位。有了线程之后，进程会成为除了CPU之外（如打印机等）的资源分配最小单位，同一进程的不同线程共享进程的资源。

线程之间切换，系统开销小；进程之间切换，系统开销大。此外同一进程中的线程间通信甚至无需系统干预。

线程是并发，进程是并行。

## 2 进程间通信的方式

**常见的有三种方式：共享存储、消息传递、管道通信。**

**每个进程在存储区的空间独立，不能够直接访问，只有通过操作系统的控制才能实现。所以我们引入了进程通信。**

![21](./imgs/21.png)

**共享存储**

在通信的进程之间存在一块可以直接访问的共享空间，通过这片共享空间进行读写。在对共享空间进行读写时，需要使用PV这种同步互斥工具。

它又分成两种：基于数据结构的共享（低级）和基于存储区的共享（高级）。操作系统只负责为通信进程提供可共享使用的存储空间和同步互斥工具，而数据交换则由用户自己安排读/写指令完成。

**消息传递**

在消息传递系统中，进程间的数据交换以**格式化的消息（Message）** 为单位。如果通信的进程不存在可以直接访问的共享空间，**则必须利用操作系统提供的消息传递方法实现进程通信。它是当前应用最广泛的进程间通信机制。**

消息传递也具有两种方式，**直接通信和间接通信。** 直接通信是指消息的发送进程要直接指明接收进程的PID，而间接通信是指消息的发送者和接收者之间通过一个或多个中间实体（“信箱”）来传递消息。

![18](./imgs/18.png)

用上面的这一张图来说明直接通信。进程P想要给进程Q发送消息，那么P首先要在自己的地址空间中创建一个**格式化的消息**，这个消息包含了消息头以及消息体，接着，通过调用发送原语`send(Q, msg)`将消息**点名道姓地**发送给Q，操作系统会把它挂在进程Q的消息队列上。如果Q需要用到这个消息，那么Q会通过调用接收原语`receive(P, &msg)`来接收消息。接收完毕后，操作系统会把这个消息复制到进程Q的地址空间中，并从Q的消息队列中删除这个消息。

![19](./imgs/19.png)

对于间接通信方式，进程P如果想要给进程Q发送消息，那么它会向操作系统申请一个**信箱**，接着，P准备好**格式化的消息**msg，使用发送原语`send(A, msg)`向信箱A发送消息msg。下面Q接收消息，首先要调用接收原语`receive(A, &msg)`，接着，操作系统会把这个消息从信箱A中取出，复制到Q的地址空间中，并从信箱A中删除这个消息。**进程P和进程Q之间并没有点名道姓，而是使用了一个公共的信箱，因此叫做间接通信。**

**管道通信**

![20](./imgs/20.png)

“管道”是一个特殊的共享文件，又名pipe文件。其实就是在内存中开辟一个大小固定的内存缓冲区。**它有点像共享存储的消息传递方式，但与它实际上有着不同。** 共享存储方式往往在那片存储区内可以任意读写，但是管道通信却对读写具有十分严格的限制。它一般会按照先进先出（FIFO）方式，像压缩饼干一样把数据压缩进去。

**一个管道只能采用半双工通信，某一时间段内只能实现单向的传输。如果要实现双向同时通信，则需要设置两个管道。**

## 3 进程同步与进程互斥

**同步：** 同步的意思就是，进程B需要使用到进程A产出的数据，它必须等待进程A执行完毕后再执行。是一种**直接制约关系**。

**互斥：** 互斥的意思就是，进程B和进程A需要同时访问一块共享资源，但是现在A已经再访问这块资源了，那么它就必须等待A访问完毕后在进行访问。是一种**间接制约关系**。

![22](./imgs/22.png)

**空闲让进、忙则等待、有限等待、让权等待。**

实现进程同步的方法：**事件（wait/notify）、信号量（PV/semaphore）、互斥锁（mutex）** 等。

**事件：** 通过通知操作的方式保持多线程同步，当一个线程完成了一定的操作，需要通知其他线程，其他线程收到通知后开始或继续工作。

**互斥锁**

解决临界区最简单的工具就是互斥锁。一个进程在进入临界区时应获得锁；在退出临界区时释放锁。一个进程在进入临界区时，如果发现锁已经被其他进程占用，那么它就会进入睡眠状态，直到锁被释放。
...
**注意：关于浏览器的渲染过程，我们在`前端篇`中已经讲到。这里重点探讨DOM树的渲染过程，即将HTML代码进行编译得到DOM树的过程。**

在计算机识别这一堆HTML代码的过程中，借助栈来辅助标签的层级关系以及标签的匹配关系。

**遇到标签遵循下面的三条规则：**

![24](./imgs/24.png)

经过上面三条规则的翻译，我们就能最终得到一个DOM树。

## 2 内存泄漏与垃圾回收

**概念：** 垃圾回收是将不再需要的内存空间回收掉的过程，而内存泄漏是指有些内存空间不需要了但垃圾回收器无法回收的情况。

内存泄漏的根源在于垃圾回收器无法理解需要和不需要的判断，因此需要人为干预。垃圾回收器的机制是标记清除算法，而引用技术是实现细节之一。要让垃圾回收器能够回收内存，需要理解其机制。

![25](./imgs/25.png)

以这副图为例，**红色为这个应用程序的总内存，虚线部分内区域为垃圾，白色为垃圾回收器识别的垃圾，那么用黄色马克笔勾画的部分就发生了内存泄漏。**