...
1. 组件化开发思想：
2. **支持多根结点**：
3. **响应式原理**: Vue2 是 Object.defineProperty; Vue3 是 Proxy
4. Vue3编译优化：静态提升：将一些静态的节点或属性提升到渲染函数之外。它能够减少更新时创建虚拟 DOM 带来的性能开销和内存占用。
...
6. 还有一些衍生的特性，如柯里化与组合，三言两语说不清，就不阐述了，有兴趣的同学可以自己再了解了解。

# 2024.8.22

## 1 JavaScript 参数传递

1. 函数参数是基本数据类型时，函数体内复制了一份参数值、任何操作都不会影响原参数的值。
2. 函数参数是引用类型时，函数体内修改这个值的某个属性时、会对原参数进行修改。
3. 函数参数是引用类型时，函数体内直接修改这个值的引用地址、相当于创建了一个新的引用，任何操作都不会影响原参数的值。
...
function foo(o) {
    o = 2;      // 本来外面是一个对象，这里相当与直接在函数内部新建了一个o。这个o实际上建在window对象下面。
    console.log(o);     // 2
}
foo(obj);
