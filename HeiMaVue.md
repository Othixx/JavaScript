...
另外需要注意，子组件props中的属性名称，必须和父组件中传递的属性名称完全一致，否则子组件无法正常接收数据。在图中就是子组件中的`title`属性，必须和父组件中传递的`title`属性完全一致。
...
请注意，$on和$emit的事件名也是必须一致，否则没法正确订阅与传递消息。
...
它本质上就是个语法糖。实现了数据的双向绑定，例如应用在输入框上，就是`value`属性和`input`事件的合写。（复习：`:`是`v-bind`的缩写，用于实现数据变了，视图跟着变）
...
一般来说，有“2 3 3，3 4 4”公式。用的是Vue2，就搭配Vue Router 3.x版本，以及Vuex 3.x版本；用的是Vue3，就搭配Vue Router 4.x版本，以及Vuex 4.x版本。上图中的5个基本步骤需要牢记。
...
在这里我们插入一个问题，就是为什么不让Vue里面全部的数据都是响应式的？那是因为某些数据没有响应式的必要，而通过响应式的原理，我们很容易分析出来响应式数据的获取和更新会比普通数据更加吃资源，所以不设置全局响应式是Vue的刻意为之。
...
如果未来我们需要在`<script>`中使用`ref`定义的数据，需要加上`.value`来访问，而`reactive`不需要。而在模板中，`ref`定义的数据可以直接使用，不需要再加`.value`了。
...
补充：为什么需要持久化？Pinia 的 Store 数据默认仅存在于浏览器的内存中，当页面刷新、浏览器重启或关闭标签页时，内存中的数据会被清空。持久化后，数据会被保存到浏览器的本地存储中，即使页面刷新或浏览器重启，数据依然可以被恢复。
...
这部分内容我们了解了一个新的element-plus组件`el-pagination`，它用于实现分页功能。具体的使用方法可以参考官方文档。

### 文章发布修改 - 抽屉

这一部分我们了解了element-plus的`el-drawer`组件，它用于实现抽屉效果。具体的使用方法可以参考官方文档。
