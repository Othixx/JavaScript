...
仍旧看上面那张图右边，程序执行完之后，由于`result()`定义在全局范围内，导致了`fn()`函数和`count`变量一直存在于内存中，无法被垃圾回收机制回收，导致内存泄漏。**解决方法是，及时将引用闭包的变量设置为null，这样垃圾回收机制就可以回收相关的内存**。
...
此外，还可以加小括号，直接返回一个对象。（为啥要加小括号呢，是因为如果直接返回一个中括号，那人家还以为是箭头函数里面的待执行代码）
...
const sum = arr.reduce((accumulator, currentValue) => {
  // accumulator是上一个值（累加器），currentValue是当前值
  ...
  constructor(func) {
    const resolve = (result) => {
      if (this.state === PENDING) {
        // 只有在pending状态下才能改变状态，并且由于是箭头函数，这里的this恰好同上级构造函数的作用域一致，指向当前实例对象
  ...
  then(onFulfilled, onRejected) {
    ...
    onRejected =
      typeof onRejected === 'function'
        ...
        : (x) => {
            throw x
          } // 如果没有传入失败回调，设置默认值为一个抛出错误的函数，注意这个大括号一定要加
    if (this.state === FULFILLED) {
      onFulfilled(this.result) // 成功回调，这个回调函数可以接收结果
    } else if (this.state === REJECTED) {
      onRejected(this.result) // 失败回调
...
但是上面这么写还不够，还会存在一个问题（**但实际上，这个问题是之前就存在的，而不是我们在添加catch实例方法引出的。视频里讲解的有问题**）。问题就是看我们测试代码注释掉的那个地方，如果我们在Promise的执行函数中抛出了异常，那么这个异常应该被`catch`方法捕获到，但是上面这么写是无法实现的。为了解决这个问题，我们需要在构造函数中，使用`try...catch`语句捕获异常，然后调用`reject`函数传递异常。代码如下：
...
顾名思义，在英语中race是比赛的意思，那么`race`方法的含义就是“赛跑”，它接受一个可迭代对象（通常是一个数组）作为参数，返回一个新的Promise对象，这个Promise对象的状态由第一个被解决或拒绝的Promise对象决定。我们给出测试代码：
...
HMPromise.race([p1, p2, 'itheima']).then(
  (res) => {
    console.log('res:', res)
  },
  (err) => {
...
完成核心步骤的分析之后，我们直接给出实现代码，我觉得这个看着好理解，实际上非常不好写：
...
第3步需要好好理解。事实上，之所以要用resolve去包裹每一个输入，是因为输入可能不是Promise对象（比如说上面测试代码中的'itheima'），而我们需要把它们都转换成Promise对象，然后才能调用`then`方法去监听它们的状态变化。

我们还需要贴一段AI的分析，方便我们理清代码到底是怎么执行的：
...
里面的返回函数的函数名称其实无所谓，我没有改是因为复用了第一段代码，懒得改。但实际上，我们可以继续化简成这样的形式：

![alt text](image-404.png)

非常的简短，这么写虽然平时开发中很少用到，但是在很多框架的源码中为了尽可能的精简，就会这么写，比如在axios中就存在这样的写法。

### 2 设计模式（面试重点）

所谓设计模式，是指在面向对象软件设计过程中针对特定问题的简洁而优雅的解决方案。目前说到设计模式，一般指的是《设计模式：可复用面向对象软件的基础》一书中提到的23种常见的软件开发设计模式。

在JS中，我们重点掌握工厂模式、单例模式、观察者模式、发布订阅模式、原型模式、代理模式以及迭代器模式这7种设计模式。

#### 2.1 工厂模式

工厂模式是一种创建型设计模式，它提供了一种创建对象的方式，而不需要暴露对象创建的具体逻辑。

![alt text](image-405.png)

简单理解来说，就是调用这个函数，返回一个对象。

#### 2.2 单例模式

在单例模式中，单例对象整个系统需要保证只有一个存在。
...
如上图，发布订阅模式依赖事件总线，也就是中间商，这也是发布订阅模式和观察者模式的区别。我们知道，在Vue2里面，可以使用$on、$emit、$off等方法来实现发布订阅模式，在Vue3中，除了$emit以外，其他的方法都被移除，我们只能使用一些第三方的库来引入，或者索性自己手写一个来实现。
...
```javascript
$off(event) {
  this.#handlers[event] = undefined   // 清空
}
```

而对于$once，我们还是注册一个事件，只不过注册这个事件比较特殊，需要满足出发它一次之后，自动销毁，后面不能再触发。这要怎么来实现呢？我们可以调用$on来实现，只不过传入的回调函数需要改成执行一次之后自动销毁：
...
原型模式的特点是通过复制一个已经存在的实例来返回新的实例，而不是新建实例。最常用的方法见下面：

![alt text](image-411.png)

请注意，Object.create()主要是用来实现原型继承，而非拷贝对象，其中上面的Vue2重写一些数组方法就用到了这个。至于为什么要重写数组方法，目的是为了有的时候我们更新一个数组，视图能够动态的更新。

#### 2.6 代理模式

代理模式就是在访问目标对象之前，先访问一个代理对象，由代理对象来访问目标对象。一个典型的应用就是缓存代理：
